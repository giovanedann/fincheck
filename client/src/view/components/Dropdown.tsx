...
function Item({ children, className, onSelect }: ItemProps) {
  return (
    <RdxDropdown.Item
      ...
      className={
        cn(
          'min-h-[2.5rem] outline-none flex items-center p-2 text-gray-800 text-sm data-[highlighted]:bg-gray-50 cursor-pointer rounded-2xl transition-all duration-300 ease-in-out',
          className
        )}
    >
      {children}
